...
type Screen = "loading" | "welcome" | "create" | "share" | "play_intro" | "quiz" | "result";
...
export default function Page() {
  ...
  const canContinueCreate = useMemo(() => {
    return !!creatorName.trim() && questions.length >= 3 && questions.length <= 5 && questions.every((q) => q.text.trim() && q.answer !== null);
  ...
  const createQuizAndShare = async () => {
    if (!supabase || !canContinueCreate) return;
    ...
    const { error: questionError } = await supabase.from("quiz_questions").insert(payload);

    if (questionError) {
      setError("Fragen konnten nicht gespeichert werden.");
      setBusy(false);
      return;
    }
  ...
  const comparisonText = useMemo(() => {
    ...
    if (currentPercent > roundedAverage) return `Du liegst über dem bisherigen Durchschnitt von ${roundedAverage}%.`;
    if (currentPercent < roundedAverage) return `Du liegst unter dem bisherigen Durchschnitt von ${roundedAverage}%.`;
  ...
  if (screen === "loading") {
    return (
      <main style={pageStyle}>
        <div style={cardStyle}><h2>Lädt ...</h2></div>
      </main>
    );
  }
      ...
      <div style={cardStyle}>
        {screen === "welcome" && (
          <div>
            ...
            <p style={pStyle}>Erstelle ein eigenes Quiz mit 3 bis 5 Fragen oder spiele ein bereits geteiltes Quiz.</p>
            {error && <p style={errorStyle}>{error}</p>}
            {!supabase && <p style={errorStyle}>Supabase ist noch nicht verbunden. Trage zuerst die beiden NEXT_PUBLIC_SUPABASE-Umgebungsvariablen ein.</p>}
            <div style={rowStyle}>
        ...
        {screen === "create" && (
          <div>
            ...
            <p style={subStyle}>Der Name des Erstellers ist Pflicht und taucht direkt im Titel auf.</p>
            <label style={labelStyle}>Name des Quiz-Erstellers</label>
            <input value={creatorName} onChange={(e) => setCreatorName(e.target.value)} style={inputStyle} placeholder="z. B. Micha" />
              ...
              {questions.map((q, index) => (
                <div key={q.position} style={questionCardStyle}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 8 }}>
                    ...
                    <button onClick={() => removeQuestion(q.position)} style={{ ...btnGhostSmall, opacity: questions.length > 3 ? 1 : 0.45 }}>Entfernen</button>
                  </div>
                  <input
                    value={q.text}
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 10, flexWrap: "wrap" }}>
                    <button onClick={() => updateQuestion(q.position, { answer: true })} style={q.answer === true ? btnPrimarySmall : btnGhostSmall}>Wahr</button>
                    <button onClick={() => updateQuestion(q.position, { answer: false })} style={q.answer === false ? btnPrimarySmall : btnGhostSmall}>Falsch</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ ...rowStyle, marginTop: 16 }}>
              <button onClick={addQuestion} style={{ ...btnGhost, opacity: questions.length < 5 ? 1 : 0.45 }}>+ Frage hinzufügen</button>
              <button onClick={createQuizAndShare} disabled={!canContinueCreate || busy} style={{ ...btnPrimary, opacity: canContinueCreate && !busy ? 1 : 0.45 }}>
        ...
        {screen === "share" && (
          <div>
            ...
            <p style={subStyle}>Der Link ist jetzt echt und kann an andere Geräte und andere Personen verschickt werden.</p>
            <div style={titleBoxStyle}>{shareUrl}</div>
            <div style={rowStyle}>
              <button onClick={copyShareLink} style={btnPrimary}>{copied ? "Kopiert" : "Link kopieren"}</button>
              <button onClick={() => setScreen("play_intro")} style={btnGhost}>Vorschau spielen</button>
            </div>
          </div>
        )}
        ...
        {screen === "play_intro" && (
          <div>
            ...
            <input value={playerName} onChange={(e) => setPlayerName(e.target.value)} style={inputStyle} placeholder="z. B. Anna" />
            <div style={rowStyle}>
              <button onClick={beginQuiz} disabled={!playerName.trim()} style={{ ...btnPrimary, opacity: playerName.trim() ? 1 : 0.45 }}>Quiz starten</button>
            </div>
          </div>
        )}
        ...
        {screen === "result" && (
          <div>
            ...
            <p style={pStyle}>Bisheriger Gesamtdurchschnitt aller Teilnehmer: {averagePercent !== null ? `${Math.round(averagePercent)}%` : "noch keine Daten"}.</p>
